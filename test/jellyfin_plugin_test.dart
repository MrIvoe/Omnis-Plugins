...
void main() {
  ...
  Future<JellyfinPlugin> configuredPlugin({
    required http.Client client,
    String server = 'https://jellyfin.example.com',
    String username = 'alice',
    String password = 'hunter2',
  }) async {
    final plugin = JellyfinPlugin(client: client);
  ...
  group('isConfigured', () {
    test('false until server/username/password are all set', () async {
      final plugin = JellyfinPlugin(client: MockClient((r) async {
        throw UnimplementedError('should not be called');
      }));
      expect(plugin.isConfigured, isFalse);

      await plugin.setServerUrl('https://jellyfin.example.com');
      expect(plugin.isConfigured, isFalse);
  ...
  group('search', () {
    ...
    test('authenticates automatically before the first search, then '
      ...
      expect(track.id, 'jellyfin:i1');
      expect(track.title, 'Song One');
      expect(track.type, TrackType.jellyfin);
    ...
    test('an item with no Id or no Name is skipped rather than returning '
      ...
      expect(result, hasLength(1));
      expect(result.single.id, 'jellyfin:good-1');
    });
  ...
  test('plugin metadata is well-formed', () {
    final plugin = JellyfinPlugin();
    expect(plugin.id, 'jellyfin');
