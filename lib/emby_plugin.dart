...
///
/// Like `JellyfinPlugin`/`OpenSubsonicPlugin`/`PlexPlugin`, and unlike
/// `SpotifyImportPlugin`'s metadata-only imports, this is a **real,
/// directly playable** streaming provider — each track's
/// [BaseTrack.streamUrl] is Emby's own real audio endpoint, playable
/// through the exact path Radio/YouTube/OpenSubsonic tracks already use
/// (`AudioEngine.uriFor` plays any track with a `streamUrl` set).
///
/// A separate plugin from `JellyfinPlugin` rather than a shared client
/// — Emby and Jellyfin are close cousins (Jellyfin began in 2018 as a
/// fork of Emby, and still uses Emby's own `X-Emby-Authorization`
/// header name for backward compatibility), so the request/response
/// shapes below (`/Users/AuthenticateByName`, `/Items` search,
/// `RunTimeTicks`, `/Audio/{id}/stream`) genuinely are the same ones
/// `JellyfinPlugin` uses. They're still kept as two distinct plugins
/// because they're two different servers a user could each
/// independently run — the same "one plugin per real server a user
/// might have, even where protocols overlap" reasoning that already
/// applies to `OpenSubsonicPlugin` covering both Subsonic and
/// Navidrome with one client, while Jellyfin/Plex/DLNA each got their
/// own despite occasional shape overlap.
///
...
/// in this plugin's own `PluginStorage`.
///
/// **This has not been exercised against a real Emby server in this
...
/// modeled on Emby's own published REST API and on the equivalent,
/// tested `JellyfinPlugin` request shapes, not a live server round-trip.
...
class EmbyPlugin extends MusicPlugin implements IOnlineSearchProvider {
  ...
  /// A per-install id Emby's auth header wants, to label this "device"
  /// in its admin UI — generated once per plugin instance rather than
  /// persisted, since it needs no stability across app restarts for
  /// correctness (only cosmetic effect on how many "devices" a
  /// long-lived install shows there over time).
  ...
  EmbyPlugin({http.Client? client, Random? random})
  ...
  String get description =>
      'Connect to a self-hosted Emby media server and stream '
      'directly from it.';
  ...
  dynamic uiSlot(String locationID) =>
      locationID == 'plugin_settings' ? _EmbySettings(plugin: this) : null;
...
class _EmbySettings extends StatefulWidget {
  final EmbyPlugin plugin;

  const _EmbySettings({required this.plugin});

  @override
  State<_EmbySettings> createState() => _EmbySettingsState();
}

class _EmbySettingsState extends State<_EmbySettings> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          'Connect to any Emby server you already run.',
